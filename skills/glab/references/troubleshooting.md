...
**Cause:**
- Target GitLab/API endpoint rejects the submitted content type for `--input`.

**Solutions:**
1. Prefer form fields:
   ```bash
   glab api --method PUT "projects/<id>/issues/<iid>" \
     --field description="Updated text"
   ```
2. Validate endpoint behavior with a small update before bulk operations.
3. Keep a fallback path: `--input` first, then retry with `--field`.
