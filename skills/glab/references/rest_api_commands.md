...
# Reply to a comment (threaded discussion) - Issue
# 1. Get discussion ID
...
# Reply to a comment (threaded discussion) - MR
# 1. Get discussion ID
DISC_ID=$(glab api "projects/namespace%2Fproject/merge_requests/456/discussions" | jq -r '.[0].id')
# 2. Reply to discussion
glab api --method POST "projects/namespace%2Fproject/merge_requests/456/discussions/$DISC_ID/notes" \
  --field body="Reply text"
...
# Retry a named job in a specific pipeline
...
glab job artifact <refName> <jobName> -p path/to/download
```
