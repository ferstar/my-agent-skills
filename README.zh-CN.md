...
`harness-observe` 使用一份本地 append-only JSONL 作为唯一事实来源，构建默认
隐私安全的持续改进闭环。它记录 workflow outcome、终态验证、skill 使用、权限
判断、标准化失败类型、耗时和 token 计数，但不保存 prompt、回复、命令、路径、
URL 或 tool payload。

Codex lifecycle 通过确定性的 `UserPromptSubmit` 和 `Stop` hooks 接入，而不是依赖
全局 prompt 要求模型记得写日志。Hook adapter 会忽略 prompt 和 assistant message
正文，也不会从最终文案猜测任务是否成功。

汇总报表和重复失败候选都可以从事件流重新生成。候选必须回到源 session 确认真正
根因，并提升为脱敏、可复现的 eval，之后才可以作为修改 harness 的依据；原始事件
不能自动授权自我修改。
