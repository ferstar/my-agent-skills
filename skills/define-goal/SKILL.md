---
name: define-goal
description: Turn an explicitly requested goal or fuzzy objective into a concrete, measurable outcome and create it with the goal tool when asked. Use only for goal definition or refinement; do not force goal creation onto ordinary implementation, planning, checkpoint, or handoff work.
---
...
5. Check active goal state before creating a goal.
   ...
   Goal status is not a workflow phase. Keep `DISCOVER -> DECIDE -> IMPLEMENT -> VERIFY -> SHIP -> DONE` checkpoints in the active task workflow; this skill only defines the measurable objective.
